...
def inserta(product_id, name, description, company, price, units, subcategory_id, created_at, updated_at):
    ...
    try:
        ...
        cur.execute(f"""INSERT INTO public.product(
	            ...
	            VALUES ({product_id}, '{name}', '{description}', '{company}', {price}, {units}, {subcategory_id}, {created_at}, {updated_at});
                """)
...
def edit(id,prod: Product):
    ...
    try:
        ...
        cur.execute(f"""UPDATE public.product
                SET name='{prod.name}', description='{prod.description}', company='{prod.company}', price={prod.price}, units={prod.units}, subcategory_id={prod.subcategory_id},  updated_at= CURRENT_TIMESTAMP
                WHERE product_id = {id};
                """)
...
def borrar(id):
    ...
    return f"Producte elimina corret"
